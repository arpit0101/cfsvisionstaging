...
class Save extends \Magento\Backend\App\Action
{
    ...
    public function execute()
    {
        ...
        if ($data) {
			...
			try {
				$uploader = $this->_fileUploaderFactory->create(['fileId' => 'region_image']);
                ...
                $path = $fileSystem->getDirectoryRead(\Magento\Framework\App\Filesystem\DirectoryList::MEDIA)->getAbsolutePath('region/'.$model->getId().'/');
                $results = $uploader->save($path);
				//echo "<pre>"; print_r($results); exit;
				$filename = $results['file'];
				if ($filename) {
					$model->setData('region_image', 'region/'.$model->getId().'/'.$filename);
					try {
						$model->save();
						/* if($image){
