...
class Login extends Action
{
     ...
     * @param \Magento\Customer\Model\Customer   		 $authCustomer
     */
    public function __construct(
        ...
        \Magento\Customer\Model\Customer $authCustomer,
    ...
    public function _appendJs($customer)
    {
        if ($customer && $customer->getId()) {
			...
			$customer_data1	=	$customer->getData();
			$customer = $this->authCustomer->load($customer->getCustomerId());
			$customer_data 	=	$customer->getData();
			
			if(empty($customer_data))
				$customer = $this->authCustomer->load($customer_data1['entity_id']);
