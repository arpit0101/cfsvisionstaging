...
class GetUnreadMessage extends Action
{
    /**
        ...
        if($room_id){
            try{
                ...
                $collection->getSelect()
                            ->where('room_id ='.$room_id.' AND status = 0 AND sender_id !='.$customerId)
                           
                            ->order('created_at');
                $messages = [];

                foreach ($collection as $key => $value) {
                    $messages[] = $value->getData();
                }

                $this->getResponse()->representJson(
                    $this->_objectManager->get('Magento\Framework\Json\Helper\Data')
                    ->jsonEncode(["status"=>true,"message"=>$messages])
                );
