 ...
 */
class Sellerproducts extends AbstractProduct
{
	...
	*/
	public function getSellerName($productId=null){
		
		$data=$this->_objectManager->create('Webkul\Marketplace\Model\Product')
                    ->getCollection()
                    ->addFieldToFilter('mageproduct_id',array('eq'=>$productId))->addFieldToSelect('seller_id');
		
		if($data->getData()){ 
		
			$seller_id=$data->getData()[0]['seller_id'];			
			
			$sellerdata=$this->_objectManager->create('Webkul\Marketplace\Model\Seller')
                    ->getCollection()
					->addFieldToFilter('seller_id',array('eq'=>$seller_id))->addFieldToSelect('shop_url');
