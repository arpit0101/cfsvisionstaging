...
/**
 * Webkul Marketplace Edit Product Sale List Block
 *
 * @category    Webkul
...
class Editproductsale extends \Magento\Framework\View\Element\Template
{
    /**
    ...
    */
    ...
    {
        parent::_construct();
        $this->pageConfig->getTitle()->set(__('Marketplace Edit Product Sale List'));
    }
    ...
    public function getSalesdetail($product_id='')
    {
        $data = array(
        ...
        $sum=0;
        $arr=array();
        $quantity = $this->_objectManager->create('Webkul\Marketplace\Model\Saleslist')
                        ->getCollection()
                        ->addFieldToFilter(
                            'mageproduct_id',
                            ['eq' => $product_id]
