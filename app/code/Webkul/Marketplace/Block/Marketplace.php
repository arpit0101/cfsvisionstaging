...
class Marketplace extends \Magento\Framework\View\Element\Template
{
    ...
    */
    ...
    {
        $marketplace_userdata = $this->_objectManager->create('Webkul\Marketplace\Model\ResourceModel\Seller\Collection')->getTable('marketplace_userdata');
        $catalog_product_entity_int = $this->_objectManager->create('Webkul\Marketplace\Model\ResourceModel\Seller\Collection')->getTable('catalog_product_entity_int');
        $marketplace_product = $this->_objectManager->create('Webkul\Marketplace\Model\ResourceModel\Seller\Collection')->getTable('marketplace_product');
        ...
        $sellers_order = $this->_objectManager->create('Webkul\Marketplace\Model\Orders')
                                ->getCollection()
        ...
        ->join(array("ccp" => $marketplace_userdata),"ccp.seller_id = main_table.seller_id",array("is_seller" => "is_seller"))->where("main_table.invoice_id!=0 AND ccp.is_seller = 1");
        ...
        foreach ($sellers_order as $value) {
            if($helper->getSellerProCount($value['seller_id'])){
                ...
                $seller_products->getSelect()
                ->join(array("mpro" => $marketplace_product),"mpro.mageproduct_id = main_table.mageproduct_id",array("status" => "status"))->where("main_table.seller_id=".$value['seller_id']." AND main_table.cpprostatus=1 AND mpro.status = 1");
                ...
                if(count($seller_arr[$value['seller_id']])<3){
                    ...
                    ->join(array("cpei" => $catalog_product_entity_int),"cpei.entity_id = main_table.mageproduct_id",array("value" => "value"))->where("cpei.value=4 AND cpei.attribute_id = ".$pro_att_id." AND cpei.store_id = ".$storeId);
                    $seller_product_coll->setPageSize(3);
                    foreach ($seller_product_coll as $value) {
                        if($seller_pro_count<3){
                            array_push($seller_arr[$value['seller_id']],$value['mageproduct_id']);
                            $seller_pro_count++;
                        }
                    }
                }
        ...
        if(count($seller_arr)!=4){
            ...
            if(count($seller_arr)){
                $seller_product_coll->getSelect()
                ->join(array("mmu" => $marketplace_userdata),"mmu.seller_id = main_table.seller_id",array("is_seller" => "is_seller"))->where("main_table.status!=0 AND mmu.is_seller = 1 AND mmu.seller_id NOT IN (".implode(',',array_keys($seller_arr)).")");
            }else{
                $seller_product_coll->getSelect()
                ->join(array("mmu" => $marketplace_userdata),"mmu.seller_id = main_table.seller_id",array("is_seller" => "is_seller"))->where("main_table.status!=0 AND mmu.is_seller = 1");
            }
            ...
            foreach ($seller_product_coll as $value) {
                if(!isset($count_pro_arr[$value['seller_id']])){
                    $count_pro_arr[$value['seller_id']] = array();
                }
